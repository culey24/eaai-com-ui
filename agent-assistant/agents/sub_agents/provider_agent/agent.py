...
from .prompt import PROVIDER_AGENT_INSTRUCTION_PROMPT
...
_ATTEMPT_KEY = "_provider_fc_round"
...
def init_session_state(callback_context: CallbackContext) -> None:
    ...
    # Không gọi /learning_history (class_students). Để trống; cá nhân hoá qua dynamic profile / công cụ khác.
...
def create_agent(query: Optional[str] = None) -> Agent:
    ...
    """
    instruction = PROVIDER_AGENT_INSTRUCTION_PROMPT
    if query:
        instruction += f"""
