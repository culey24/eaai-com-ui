...
export default function SupportingChatsPage() {
  const { t } = useLanguage()
  const { user } = useAuth()
  ...
  const allUsersInClasses = managedClasses.flatMap((c) => getByClass(c))
  const users = search.trim()
    ? allUsersInClasses.filter((u) =>
        ...
        (u.classCode?.toLowerCase().includes(search.toLowerCase().trim()))
      )
    : allUsersInClasses
