    ...
    if step >= MAX_RETRIES + 2: # 1 for result from the final function call and 1 for the first exceeded step
        ...
        return LlmResponse(
            content=types.Content(
                ...
                parts=[types.Part(
                    text=(
                        "I'm sorry, I couldn't find the right information."
                        "If you have more details, I'd be happy to help you find it."
                    )
                )]
            )
        )
...
def create_agent() -> Agent:
    ...
    if os.getenv("ENABLE_PERSONA", "true").lower() == "true":
        # logger.info("Including Persona Agent in the toolset.")
        tools.append(call_persona_agent)
