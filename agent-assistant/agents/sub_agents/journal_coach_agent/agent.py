...
from .prompt import JOURNAL_COACH_AGENT_INSTRUCTION_PROMPT
...
_ATTEMPT_KEY = "_journal_coach_fc_round"
        ...
        return LlmResponse(
            content=types.Content(
                ...
                parts=[
                    types.Part(
                        text=(
                            "Rất tiếc, mình không xử lý kịp yêu cầu này. "
                            "Bạn thử gửi lại đoạn nháp hoặc câu hỏi cụ thể hơn nhé."
                        )
                    )
                ],
            )
...
def create_agent(query: Optional[str] = None) -> Agent:
    instruction = JOURNAL_COACH_AGENT_INSTRUCTION_PROMPT
    if query:
        instruction += f"""
...
"""
    ...
    return Agent(
        name="hcmut_journal_coach_agent",
        model=get_adk_model(),
        ...
        output_key="journal_coach_infos",
