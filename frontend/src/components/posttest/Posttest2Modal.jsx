...
import { SECTION_C_ITEMS } from '../../data/posttest2/sectionCItems'
import { getSectionBQuestions } from '../../data/posttest2/sectionBQuestions'
...
function emptyBlock15() {
  return Object.fromEntries(Array.from({ length: 15 }, (_, i) => [`q${i + 1}`, '']))
}
...
export default function Posttest2Modal({ onComplete, onCancel }) {
  ...
  // Auto-fill Section A if user already did pretest
  useEffect(() => {
    if (apiToken) {
  ...
  const ensureB = useCallback(
    (a) => {
      ...
      setSectionB((prev) => ({
        ...prev,
        [u1]: prev[u1] || emptyBlock15(),
        [u2]: prev[u2] || emptyBlock15(),
      }))
    },
    [setSectionB]
  )
  ...
  const submit = async () => {
    ...
    setSending(true)
    try {
      const res = await fetch(`${API_BASE}/api/me/posttest2`, {
        method: 'POST',
        headers: {
  ...
  return (
    <div className="fixed inset-0 z-[300] flex flex-col bg-white dark:bg-slate-900 overflow-hidden">
      <header className="flex-shrink-0 border-b border-slate-200 dark:border-slate-700 px-6 py-4 flex items-center justify-between">
        <div>
          <h1 className="text-xl font-bold text-slate-900 dark:text-white">Post-course Survey 2 (Post-test 2)</h1>
      ...
      <div className="flex-1 overflow-y-auto px-6 py-6">
        ...
        {step === 0 && (
          <div className="max-w-2xl space-y-4">
            <p className="text-lg text-slate-800 dark:text-slate-200">Welcome to the Post-test 2 assessment.</p>
            <p className="text-slate-600 dark:text-slate-400">This survey evaluates your knowledge retention and long-term attitudes. Your responses are highly valuable for our ongoing research.</p>
        ...
        <div className="flex gap-3">
          ...
          {step === 4 && (
            <button onClick={submit} disabled={sending} className="px-6 py-2 bg-primary text-white rounded-xl disabled:opacity-50">
              {sending ? 'Submitting...' : 'Submit Post-test 2'}
            </button>
          )}
        </div>
      </footer>
